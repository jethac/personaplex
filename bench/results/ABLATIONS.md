...
| **--fast** (w8a16+depq8+skipother+mimi-fp16+fusion; preset) | **46.47** | **47.58** | **48.11** | 48.11 | 0/475 |
| fast with fp32 mimi (variant) | 48.56 | 49.58 | 50.40 | 50.47 | 0/475 |
| --fast --nvfp4-ffn (opt-in stack) | 41.78 | 43.16 | 43.79 | 44.05 | 0/475 |
...
| --fast (preset) | 43.16 | 44.26 | 44.70 |
| fast with fp32 mimi (variant) | 43.45 | 44.48 | 45.05 |
| --fast --nvfp4-ffn | 38.39 | 39.70 | 40.15 |

--fast composition: --w8a16 --dep-q-exit 8 --skip-other-mimi --mimi-fp16
plus the rms_norm torch_compile_lazy fusion (in-tree). mimi-fp16 was
briefly demoted on a single-trial listening report of a softened onset;
a 5-pair blind matched-pairs A/B (20260724-audio-v3/) scored 0/5
within-pair discrimination — the onset character tracks the SEED, not
mimi precision — so the preset was reinstated per the pre-registered
rule. The fp32-mimi variant row is kept above for the record. w8a16 is the
